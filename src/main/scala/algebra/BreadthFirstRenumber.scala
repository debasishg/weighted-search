...
object BreadthFirstRenumber extends App with Ops {
  ...
  /**
   * The `renumber` function numbers the nodes of a tree in breadth-first order. 
   * First, the `bft` function applies the effectful action num to every value in the tree. 
   * This action produces an effect in the state monad: the state in question here is an 
   * integer counter, which is first incremented using `modify` and then the updated value is 
